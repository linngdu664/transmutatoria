...
//        System.out.println(tagToItems);
        ...
        // 以下为炼金转化
        // 精确配方
        registryAccess.lookup(InitDatapacks.ALCHEMICAL_TRANSFORMATION_PRECISE_KEY).ifPresent(transLookup -> {
            HashMap<ItemStackTemplate, JEIAlchemicalTransformationDisplay> itemStackToTransDisplay = new HashMap<>();
            // 顺序遍历所有数据包配方
            transLookup.listElements().map(Holder.Reference::value).forEach(recipe -> {
                ItemStack outputItemStack = recipe.getOtherSideItemStack();
                // 不允许输出空气
                if (!outputItemStack.isEmpty()) {
                    if (recipe.isValid()) {
                        itemStackToTransDisplay.put(recipe.input(), new JEIAlchemicalTransformationDisplay(recipe, recipe.input().create(), outputItemStack));
                    } else {
                        itemStackToTransDisplay.remove(recipe.input());
                    }
                }
            });
            registration.addRecipes(AlchemicalJeiTypes.ALCHEMICAL_TRANSFORMATION, itemStackToTransDisplay.values().stream().sorted().toList());
        });
        ...
        registryAccess.lookup(InitDatapacks.ALCHEMICAL_TRANSFORMATION_KEY).ifPresent(transLookup -> {
            ...
            transLookup.listElements().map(Holder.Reference::value).forEach(recipe -> {
                ...
                // 不允许输出空气
                if (!outputItemStack.isEmpty()) {
                    if (recipe.inputItems().isPresent()) {
                        if (recipe.isValid()) {
                            for (Holder<Item> itemHolder : recipe.inputItems().get().getValues()) {
                                Item item = itemHolder.value();
                                itemToTransDisplay.put(item, new JEIAlchemicalTransformationDisplay(recipe, item.getDefaultInstance(), outputItemStack));
                            }
                        } else {
                            for (Holder<Item> itemHolder : recipe.inputItems().get().getValues()) {
                                Item item = itemHolder.value();
                    ...
                    } else if (recipe.inputNamespace().isPresent()) {
                        String namespace = recipe.inputNamespace().get();
                        if (recipe.isValid()) {
        ...
        // 以下为炼金复制
        // 精确配方
        registryAccess.lookup(InitDatapacks.ALCHEMICAL_REPLICATION_PRECISE_KEY).ifPresent(repLookup -> {
            HashMap<ItemStackTemplate, JEIAlchemicalReplicationDisplay> itemStackToRepDisplay = new HashMap<>();
            // 顺序遍历所有数据包配方
            repLookup.listElements().map(Holder.Reference::value).forEach(recipe -> {
                if (recipe.isValid()) {
                    itemStackToRepDisplay.put(recipe.output(), new JEIAlchemicalReplicationDisplay(recipe, recipe.getOtherSideItemStack(), recipe.output().create()));
                } else {
                    itemStackToRepDisplay.remove(recipe.output());
                }
            });
            registration.addRecipes(AlchemicalJeiTypes.ALCHEMICAL_REPLICATION, itemStackToRepDisplay.values().stream().sorted().toList());
        });
        ...
        registryAccess.lookup(InitDatapacks.ALCHEMICAL_REPLICATION_KEY).ifPresent(repLookup -> {
            ...
            repLookup.listElements().map(Holder.Reference::value).forEach(recipe -> {
                // 炼金复制允许输入空气
                ItemStack inputItemStack = recipe.getOtherSideItemStack();
                if (recipe.outputItems().isPresent()) {
                    if (recipe.isValid()) {
                        for (Holder<Item> itemHolder : recipe.outputItems().get().getValues()) {
                            Item item = itemHolder.value();
                            itemToRepDisplay.put(item, new JEIAlchemicalReplicationDisplay(recipe, inputItemStack, item.getDefaultInstance()));
                        }
                    } else {
                        for (Holder<Item> itemHolder : recipe.outputItems().get().getValues()) {
                            Item item = itemHolder.value();
                ...
                } else if (recipe.outputNamespace().isPresent()) {
                    String namespace = recipe.outputNamespace().get();
                    if (recipe.isValid()) {
                        for (Item item : namespaceToItems.getOrDefault(namespace, new ArrayList<>())) {
                            itemToRepDisplay.put(item, new JEIAlchemicalReplicationDisplay(recipe, inputItemStack, item.getDefaultInstance()));
                        }
