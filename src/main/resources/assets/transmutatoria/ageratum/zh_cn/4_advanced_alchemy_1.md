...
- **印记卷**：用于**炼金复制**——以一件物品为模板，复制出一份新的。
- **方程卷**：用于**炼金转化**——将一种物品转化为另一种。
...
- **左侧物品栏**：从上到下依次显示催化剂、输入物品、配方揭示进度和输出物品。最左侧长条表示卷轴的耐久与稳定度。
- **中央槽位图**：彩色边框标出当前选中的源质槽位。已揭示槽位会显示目标源质。按住 Shift 滚动鼠标滚轮可以切换选中槽位。
- **顶部源质轮盘**：手持储物盒时显示当前选中的源质。滚动鼠标滚轮切换源质，右键炼金锅即可投入。
- **右上角表盘**：外侧指针表示锅的当前极性，两个内侧指针表示配方允许的极性范围。沙漏显示卷轴的过期倒计时。
...
按住 **Alt** 会将轮盘向下展开，并以当前源质为基准显示它与其他源质的关系。图中的“克制”表示当前源质克制该源质，“被克制”表示当前源质受该源质克制。松开 Alt 后，轮盘会重新收起。

### 操作步骤

1. **投入卷轴**：将已激活的卷轴丢入炼金锅，进入催化剂槽。HUD 中央随即显示卷轴的源质槽位图。
...
**只有当所有源质全部湮灭**，输出槽才可能产出最终物品。未完全湮灭则反应失败——填入的源质变为反应后的状态留在槽位中，你可以取出后重新尝试。
